...
async function bulkMoveIssues(creds, sourceProject, targetProject, jql) {
  ...
  const data = await jiraFetch(creds, "/rest/api/3/search/jql", {
    method: "POST",
    body: JSON.stringify({ jql: resolvedJql, fields: ["summary", "issuetype"], maxResults: 1e3 })
  });
  ...
  for (const issue of issues) {
    if (!typeByName[issue.issuetype]) {
      throw new Error(
        `Target project ${targetProject} has no issue type "${issue.issuetype}". Available: ${targetProjectData.issueTypes.map((it) => it.name).join(", ")}`
      );
    }
...
async function main() {
  ...
  const result = await bulkMoveIssues(creds, opts.from, opts.to, opts.jql);
