...
export async function confluenceFetch(creds, path, opts = {}) {
    const auth = Buffer.from(`${creds.email}:${creds.token}`).toString('base64');
...
// ── Jira REST API v3 ─────────────────────────────────────────────────────────
export async function fetchProjects(creds) {
...
export async function fetchIssuesByJQL(creds, jql, maxResults = 50, fields = ISSUE_LIST_FIELDS) {
    let data;
    try {
...
export async function fetchIssueDetail(creds, key) {
 ...
 */
export async function fetchIssueRaw(creds, key) {
    const fields = [
        'summary', 'description', 'status', 'priority', 'issuetype',
        'assignee', 'reporter', 'created', 'updated', 'duedate',
...
export async function fetchIssuesForProject(creds, project, jql) {
...
// ── Write operations ─────────────────────────────────────────────────────────
export async function getTransitions(creds, key) {
...
export async function transitionIssue(creds, key, transitionId, comment) {
...
export async function assignIssue(creds, key, accountId) {
...
export async function addComment(creds, key, bodyMarkdown) {
...
export async function createIssue(creds, opts) {
    const fields = {
    ...
    if (opts.description)
        fields.description = markdownToAdf(opts.description);
...
export async function updateIssue(creds, key, updates) {
    const fields = {};
...
// ── Agile API v1 ─────────────────────────────────────────────────────────────
export async function fetchBoards(creds, projectKey) {
...
export async function fetchSprints(creds, boardId, state) {
...
export async function fetchSprintIssues(creds, sprintId, maxResults = 50) {
    const data = await jiraFetch(creds, `/rest/agile/1.0/sprint/${sprintId}/issue?fields=${ISSUE_LIST_FIELDS.join(',')}&maxResults=${maxResults}`);
    return (data?.issues ?? []).map(formatIssueSummary);
}
export async function createSprint(creds, boardId, opts) {
...
export async function updateSprint(creds, sprintId, fields) {
    const body = {};
...
export async function deleteSprint(creds, sprintId) {
    await jiraFetch(creds, `/rest/agile/1.0/sprint/${sprintId}`, { method: 'DELETE' });
    return { ok: true };
}
export async function moveIssuesToSprint(creds, sprintId, issueKeys) {
...
export async function moveIssuesToBacklog(creds, issueKeys) {
    await jiraFetch(creds, '/rest/agile/1.0/backlog/issue', {
        method: 'POST', body: JSON.stringify({ issues: issueKeys }),
    });
    return { ok: true, moved: issueKeys.length };
}
export async function rankIssues(creds, issueKeys, opts) {
...
// ── Issue links ──────────────────────────────────────────────────────────────
export async function fetchLinkTypes(creds) {
...
export async function linkIssues(creds, sourceKey, targetKey, linkTypeName = 'Relates') {
...
// ── Project helpers ──────────────────────────────────────────────────────────
export async function fetchProjectWithIssueTypes(creds, projectKey) {
...
export async function bulkMoveIssues(creds, sourceProject, targetProject, jql) {
